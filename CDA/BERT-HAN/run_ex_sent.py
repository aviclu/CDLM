...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        result = compute_metrics("mrpc", [p[1] for p in preds], out_label_ids)
...
def test(args, model, tokenizer, prefix=""):
    ...
    results = {}
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples_test(args, eval_task, tokenizer)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    all_input_ids = [tokenizer.encode('<g>', add_special_tokens=False) + x.input_ids[0] + x.input_ids[1] for x in features]
    all_input_ids = [x+ [1]*max(0,4096-len(x)) for x in all_input_ids]
    # all_input_ids = torch.tensor(all_input_ids, dtype=torch.long)
    # Convert to Tensors and build dataset
    #print(max([max(len(x.input_ids[0]),len(x.input_ids[1])) for x in features]))
    #exit(0)
    #all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
    #all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long)
    #all_token_type_ids = torch.tensor([f.token_type_ids for f in features], dtype=torch.long)
    all_labels = torch.tensor([f.label for f in features], dtype=torch.float)
    all_labs = [f.label for f in features]

    data = [[torch.tensor(all_input_ids[i], dtype=torch.long), torch.tensor(all_labs[i], dtype=torch.long)] for i in range(len(all_labels))]
...
def load_and_cache_examples_test(args, task, tokenizer):
    ...
    if args.local_rank == 0 and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    tokenizer.add_tokens(['<g>'], special_tokens=True)
    all_input_ids = [tokenizer.encode('<g>', add_special_tokens=False) + x.input_ids[0] + x.input_ids[1] for x in
                     features]
    all_input_ids = [x + [1] * max(0, 4096 - len(x)) for x in all_input_ids]
    # all_input_ids = torch.tensor(all_input_ids, dtype=torch.long)
    # Convert to Tensors and build dataset
    ...
    data = [[torch.tensor(all_input_ids[i], dtype=torch.long), torch.tensor(all_labs[i], dtype=torch.long)] for i in
