...
if __name__ == '__main__':
    ...
    subdirs = ['checkpoint_9','checkpoint_8','checkpoint_7','checkpoint_6','checkpoint_5','checkpoint_4','checkpoint_3','checkpoint_2','checkpoint_1']
    ...
    sys_file = 'data/ecb/gold/test_{}_topic_level.conll'.format(config['mention_type'])

    data = CrossEncoderDatasetFull(config, 'test')
    ...
    for subdir in subdirs:
        logger.info('Running: {}'.format(subdir))
        path = os.path.join(par_path,subdir)
        cross_encoder.model = AutoModel.from_pretrained(os.path.join(path, 'bert')).to(device)
        cross_encoder.linear.load_state_dict(torch.load(os.path.join(path, 'linear')))
        ...
        clustering_75 = AgglomerativeClustering(n_clusters=None, affinity='precomputed', linkage=config['linkage_type'],
                                             distance_threshold=0.75)
        clustering_8 = AgglomerativeClustering(n_clusters=None, affinity='precomputed', linkage=config['linkage_type'],
                                             distance_threshold=0.8)
        clustering_85 = AgglomerativeClustering(n_clusters=None, affinity='precomputed', linkage=config['linkage_type'],
                                             distance_threshold=0.85)
        clustering_9 = AgglomerativeClustering(n_clusters=None, affinity='precomputed', linkage=config['linkage_type'],
                                             distance_threshold=0.9)

        gold_clustering = clustering_7
        clustering = [clustering_5, clustering_55, clustering_6, clustering_65, clustering_7, clustering_65, clustering_8, clustering_85, clustering_9]
        clusters = [list(), list(), list(), list(), list(), list(), list(), list(), list()]
        max_ids = [0, 0, 0, 0, 0,0,0,0,0]
        threshold = {id: thresh for id, thresh in enumerate([0.5, 0.55, 0.6, 0.65, 0.7,0.75,0.8,0.85,0.9])}
        ...
        for topic, topic_num in enumerate(list(set([x['subtopic'] for x in data.mentions]))):
            ...
            for i in tqdm(range(0, len(instances), b_size)):
                batch = [instances[x] for x in range(i, min(i + b_size, len(instances)))]
                if not cdmlm:
        ...
        for i, predicted in enumerate(clusters):
            ...
            doc_name = '{}_{}_{}_{}'.format(
                'test', config['mention_type'], config['linkage_type'], threshold[i])
            save_path = path
